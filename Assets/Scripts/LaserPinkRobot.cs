...
public class LaserPinkRobot : MonoBehaviour {


	private Color colorLaser = new Color(1f,0.43f,0.71f);
	...
	void Update () {
		...
		if (Physics.Raycast (transform.position, transform.forward, out collisionPoint)) {
			if (isActive) {
				...
				lastHitted.SendMessage ("isBeingHit", "pink");
			}
		} else {
			if (isActive) {
				shutdown ();
	...
	void shutdown() {
		if (hasHitten) {
			lastHitted.SendMessage ("isNotBeingHit", "pink");
		}
		hasHitten = false;
	}
}
