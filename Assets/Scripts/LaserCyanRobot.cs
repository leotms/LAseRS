...
public class LaserCyanRobot : MonoBehaviour {

	private Color colorLaser = Color.cyan;
	...
	void Update () {
		...
		if (Physics.Raycast (transform.position, transform.forward, out collisionPoint)) {
			if (isActive) {
				...
				lastHitted.SendMessage ("isBeingHit", "cyan");
			}
		} else {
			if (isActive) {
				shutdown ();
	...
	void shutdown() {
		if (hasHitten) {
			lastHitted.SendMessage ("isNotBeingHit", "cyan");
		}
		hasHitten = false;
	}
}
